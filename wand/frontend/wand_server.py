...
class WandServer:
    def __init__(self):
        ...
        if self.config["switch"]["type"] == "internal":
            ...
            self.switch = LeoniSwitch(self.config["switch"]["ip"], args.simulation)
        elif self.config["switch"]["type"] is None:
            self.switch = None
        else:
            raise ValueError(
                "Unrecognised switch type: {}".format(self.config["switch"]["type"])
            )
        ...
        self.osa_db = Notifier(
            {laser: {"trace": None, "timestamp": 0} for laser in self.lasers}
        )
        ...
        self.server_control = RPCServer(
            {"control": self.control_interface}, allow_parallel=True
        )
    ...
    def start(self):
        ...
        loop.run_until_complete(self.server_control.start(bind, self.args.port_control))
        atexit_register_coroutine(self.server_control.stop)

        # start notify server
        loop.run_until_complete(self.server_notify.start(bind, self.args.port_notify))
        ...
        while self.running:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"failed to clean up connection to '{laser}'", exc_info=True
                    )
                    pass
    ...
    async def measurement_task(self):
        ...
        while True:
            ...
            if laser_conf.get("osa", "wlm") == "wlm":
                ...
                wlm_data, osa = (
                    await asyncio.gather(freq_measurement, osa_measurement)
                )[:]
            ...
            for task in self.queue:
                if task["laser"] == laser and (
                    meas["get_osa_trace"] or not task["get_osa_trace"]
                ):
                    task["done"].set()
    ...
    def take_osa_measurement(self, laser, osa, get_osa_trace):
        ...
        if not get_osa_trace:
            return {"trace": None, "timestamp": time.time()}

        osa = {"trace": self.osas.get_trace(osa).tolist(), "timestamp": time.time()}
        return osa
    ...
    def take_freq_osa_measurement(self, laser, f0, get_osa_trace):
        ...
        if not get_osa_trace:
            osa = {"trace": None, "timestamp": time.time()}
        else:
            osa = {"trace": self.wlm.get_pattern().tolist(), "timestamp": time.time()}
